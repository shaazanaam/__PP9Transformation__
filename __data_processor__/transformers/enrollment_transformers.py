"""
Enrollment data transformations.

This module handles all enrollment-related transformations:
- Statewide V01
- Tri-County
- County-Layer
- Metopio Statewide
...
class EnrollmentTransformers:
    """
    Handles all enrollment-related data transformations.
    """
    ...
    def transform_statewide(self):
        ...
        data = SchoolData.objects.filter(school_name='[Statewide]').select_related('stratification')
        transformed_data = []
    ...
    def apply_tri_county_layer_transformation(self):
        ...
        try:
            ...
            # Fetch filtered school data with related stratification (PERFORMANCE OPTIMIZATION)
...
# Apply the county Layer Transformation 
    ...
    def apply_county_layer_transformation(self):
        try:
            ...
            # STEP 2: Fetch dataset with related data (PERFORMANCE OPTIMIZATION)
            school_data = SchoolData.objects.filter(
    ...
    def transform_Metopio_StateWideLayer(self):
        ...
        try:
            ...
            school_data = SchoolData.objects.filter(district_name=district_name_filter).select_related('stratification')
            logger.info(f"Filtered school data count: {school_data.count()}")
